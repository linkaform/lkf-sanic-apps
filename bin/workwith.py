#!/usr/bin/env python3
# coding: utf-8
"""workwith: elige la cuenta activa y deja app/modules en su rama.

Corre en el HOST, no dentro del contenedor: app/modules es un submodulo cuyo .git real
vive en ../.git/modules/modules, que no esta montado, asi que git no funciona ahi adentro.

No hace login ni toca la red mas que un fetch, a proposito: tiene que servir justamente
cuando las credenciales activas estan rotas.
...
# A diferencia de addons, aqui el submodulo cuelga de app/.
MODULES = os.path.join(REPO, 'app', 'modules')

SEC_GLOBAL = 'global'
# Las ramas de cuenta salen de aqui cuando todavia no existen.
BASE = 'origin/master'
...
def git(*args, check=True):
    """git dentro de app/modules. Devuelve stdout; lanza Error si falla y check."""
...
def cargar_catalogo():
    if not os.path.exists(ACCOUNTS):
        raise Error(
            'No existe el catalogo de cuentas: %s\n\n'
            'Mientras dura la migracion ese archivo es un symlink a\n'
            '~/lkf/addons/secrets/accounts.ini, asi que necesitas el repo addons\n'
            'clonado como hermano de este. Si no lo tienes, copia\n'
            'secrets/accounts.ini.example a secrets/accounts.ini y llenalo.' % ACCOUNTS)
...
def exigir_rama_limpia():
    ...
    if sucio:
        ...
        msg = ['app/modules tiene %d cambio(s) sin commitear:' % len(archivos), '']
        ...
        msg += ['',
                ...
                '',
                '    cd app/modules && git stash        # los recuperas con: git stash pop',
                '    cd app/modules && git commit -am "..."']
        raise Error('\n'.join(msg))


def ir_a_rama(rama):
    """Cambia a la rama. Si no existe en ningun lado, la crea desde origin/master.

    Las ramas de cuenta de este repo salen de master, y hoy la mayoria todavia no
    existe: crearla aqui evita tener que acordarse del comando. No se hace push --
    publicar en el remoto compartido es decision del que la crea.
    """
    ...
    if rama in locales:
        ...
        if not git('rev-parse', '--verify', '--quiet', BASE, check=False):
            raise Error('No existe %s, no hay de donde sacar la rama "%s".' % (BASE, rama))
        git('checkout', '-b', rama, BASE)
        nueva = True
    print('  rama          : %s (antes %s)%s'
          % (rama, actual, ' [NUEVA, desde %s]' % BASE if nueva else ''))
    return nueva
...
def merge_master():
    ...
    """
    if not git('rev-parse', '--verify', '--quiet', BASE, check=False):
        print('  master        : no existe %s, se omite' % BASE)
        return []

    pendientes = int(git('rev-list', '--count', 'HEAD..%s' % BASE) or 0)
    if pendientes == 0:
    ...
    choques = []
    prueba = subprocess.run(['git', '-C', MODULES, 'merge', '--no-commit', '--no-ff', BASE],
                            capture_output=True, text=True)
    ...
    if git('diff', '--name-only', '--diff-filter=U'):
        raise Error('Quedaron conflictos sin resolver en app/modules. Resuelvelos a mano:\n'
                    '    cd app/modules && git status')
...
def mostrar_estado(cat):
    ...
    try:
        print('rama de app/modules: %s' % git('rev-parse', '--abbrev-ref', 'HEAD'))
    except Error:
        pass
...
def main(argv):
    ...
    nueva = ir_a_rama(rama)
    choques = [] if nueva else merge_master()
    ...
    if nueva:
        print('\n  OJO: la rama "%s" es local, todavia no esta en origin.' % rama)
        print('  Publicala con:')
        print('     cd app/modules && git push -u origin %s' % rama)
