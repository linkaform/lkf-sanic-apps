...
# class Expenses(base.LKF_Base):
    ...
    def update_expense_catalog_values(self, folio):
        ...
        if self.SOL_DATA.get(self.Employee.EMPLOYEE_OBJ_ID):
            catalog_data['answers'].update(self.SOL_DATA.get(self.EMPLOYEE_OBJ_ID))
        else:
            employee_data = self.Employee.get_employee_data(name=None, user_id=self.user_id)
            print('employee_data',employee_data)
        print('employee_data',employee_datad)
        #catalogo_metadata.update({'record_id': catalog_data.pop('_id'), '_rev': catalog_data['answers'].pop('_rev'), 'answers': catalog_data['answers']})
        print('self.CATALOG_SOL_VIAJE_ID=',self.CATALOG_SOL_VIAJE_ID)
        print('self.catalog_data=',catalog_data)
