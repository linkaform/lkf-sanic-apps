...
from lkf_addons.product.app import Product, Warehouse
...
class Stock(Employee, Warehouse, Product, base.LKF_Base):
    ...
    def __init__(self, settings, folio_solicitud=None, sys_argv=None, use_api=False):
        ...
        self.FORM_INVENTORY_ID = self.lkm.form_id('green_house_inventory','id')
    ...
    def do_scrap(self):
        ...
        if not stock_record:
            msg = f"No stock record form for folio: {folio_inventory}. Please check your selection"
            msg_error_app = {
                    f"{self.f['product_code']}": {
                        "msg": [msg],
    ...
    def detail_stock_dest_location_form_many(self, move_type, warehouse=None, product_code=None, lot_number=None,  location=None, date_from=None, date_to=None, status='done', **kwargs):
        match_query = {
            "deleted_at":{"$exists":False},
        ...
        if lot_number:
            unwind_query.update({f"answers.{self.f['inv_group']}.{self.CATALOG_INVENTORY_OBJ_ID}.{self.f['product_lot']}":int(lot_number)})
        if warehouse:
            if move_type == 'in':
        ...
        project = {'$project':
                {'_id': 1,
                    ...
                    'date': f"$answers.{self.f['grading_date']}",
                    'lot_number': f"$answers.{self.f['inv_group']}.{self.CATALOG_INVENTORY_OBJ_ID}.{self.f['product_lot']}",
                    'date': f"$answers.{self.f['grading_date']}",
                    'qty_out': f"$answers.{self.f['inv_group']}.{self.f['move_out']}",
                    'warehouse_from': f"$answers.{self.f['inv_group']}.{self.CATALOG_INVENTORY_OBJ_ID}.{self.f['warehouse']}",
                    'warehouse_to': f"$answers.{self.f['move_new_location']}"
                    }
            }
        query= [{'$match': match_query }]
    ...
    def process_record_to_catalog(self, current_record ):
        ...
        catalog_fields = self.lkf_api.get_catalog_id_fields( self.CATALOG_INVENTORY_ID )
        ...
        if record_catalog:
            ...
            if self.answers.get(self.f['product_lot_actuals'], 1) <= 0:
                ...
                response_delete_catalog = self.lkf_api.delete_catalog_record(self.CATALOG_INVENTORY_ID, info_record_catalog.pop('_id'), info_record_catalog.pop('_rev'))
                return True
            ...
            response_update_catalog = self.lkf_api.bulk_patch_catalog([catalogo_metadata,], self.CATALOG_INVENTORY_ID)
        else:
    ...
    def delete_cycle_count(self, folio):
        ...
        if len(list_records) > 0:
            request = self.check_status_code(self.lkf_api.patch_record_list({"deleted_objects": list_records,}))
            status_code = request.get('status_code')
            if status_code == 202:
